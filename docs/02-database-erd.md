...
> ## ℹ️ Этот документ описывает ПОЛНУЮ схему, включая будущий кабинет
>
> В `prisma/schema.prisma` сейчас **13 таблиц из 33**: содержимое сайта, услуги
> и цены, правовые документы, согласия на cookie, отзывы, видео, вход в админку,
> журнал аудита и всё, что связано с ловушками и безопасностью.
>
> Таблиц с личными данными клиентов — пользователи, записи на приём, видеовстречи,
> заметки психолога, платежи, возвраты, уведомления, GDPR-запросы — в коде **нет
> намеренно**. Они появятся вместе с личным кабинетом, на отдельном поддомене
> и отдельном сервере. На основном домене не должно быть ни одной такой таблицы,
> даже пустой: нет таблицы — нечему утечь (docs/13-site-architecture.md).
>
> Схема ниже остаётся действующим проектом — по ней будет строиться кабинет.
